/**
 * BON (Binary Object Notation) 协议实现 - 后端版本
 * 移植自 src/utils/bonProtocol.js，去除浏览器/Vue 依赖
 */
...
export class DataReader {
  ...
  validate(n) {
    if (this.position + n > this._data.length) { console.error('read eof'); return false }
    return true
  }
  ...
  readInt64() {
    const lo = this.readInt32(); if (lo === undefined) return
    let _lo = lo; if (_lo < 0) _lo += 0x100000000
    const hi = this.readInt32(); if (hi === undefined) return
    return _lo + 0x100000000 * hi
  }
  readFloat32() {
    if (!this.validate(4)) return
    const v = this.dataView.getFloat32(this.position, true); this.position += 4; return v
  }
  readFloat64() {
    if (!this.validate(8)) return
    const v = this.dataView.getFloat64(this.position, true); this.position += 8; return v
  }
  read7BitInt() {
    let value = 0, shift = 0, b = 0, count = 0
    do {
      if (count++ === 35) throw new Error('Format_Bad7BitInt32')
      b = this.readUInt8(); value |= (b & 0x7f) << shift; shift += 7
    } while ((b & 0x80) !== 0)
    return value >>> 0
  }
  readUTF() { const len = this.read7BitInt(); return this.readUTFBytes(len) }
  readUint8Array(length, copy = false) {
    const start = this.position, end = start + length
    const out = copy ? this._data.slice(start, end) : this._data.subarray(start, end)
    this.position = end; return out
  }
  readUTFBytes(length) {
    if (length === 0) return ''
    ...
    this.position += length; return str
  }
}
...
export class DataWriter {
  ...
  ensureBuffer(size) {
    ...
    const prev = _shared, need = this.position + size
  ...
  writeInt32(v) {
    this.ensureBuffer(4)
    this.data[this.position++] = v | 0; this.data[this.position++] = (v >> 8) & 0xff
    this.data[this.position++] = (v >> 16) & 0xff; this.data[this.position++] = (v >> 24) & 0xff
  }
  writeInt64(v) {
    this.writeInt32(v)
  ...
  writeUTF(str) {
    ...
    const from = this.position, reserved = from - start
    const encoder = new TextEncoder()
    const { written } = encoder.encodeInto(str, this.data.subarray(this.position))
    this.position += written
    const after = this.position, size = after - from
    this.position = start; this._write7BitInt(size)
  ...
  writeUint8Array(src, offset = 0, length) {
    const start = offset | 0, end = Math.min(src.byteLength, start + (length ?? src.byteLength))
    const n = end - start; if (n <= 0) return
    this.ensureBuffer(n); this.data.set(src.subarray(start, end), this.position); this.position += n
  }
  writeUTFBytes(str) {
...
export class BonEncoder {
  ...
  encodeString(s) {
    ...
    this.dw.writeInt8(5); this.dw.writeUTF(s); this.strMap.set(s, this.strMap.size)
  }
...
export class ProtoMsg {
  constructor(raw) {
    ...
    this._raw = raw; this._rawData = undefined; this._data = undefined; this.rtt = 0
  }
  ...
  get rawData() {
    ...
    this._rawData = bon.decode(this.body); return this._rawData
  }
  getData() { if (this._data !== undefined || this.rawData === undefined) return this._data; this._data = this.rawData; return this._data }
}
...
// xtm 在后端不支持，降级处理
const xtm = { encrypt: (e) => e, decrypt: (e) => e }

registry.set('lx', lx)
registry.set('x', x)
registry.set('xtm', xtm)
...
export default { ProtoMsg, getEnc, encode, parse, g_utils, bon }
