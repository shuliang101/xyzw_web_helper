...
const TOKEN_REFRESH_THRESHOLD = 12 * 60 * 60 * 1000; // 12小时
...
export const useTokenStore = defineStore("tokens", () => {
  ...
  const remoteBinSynced = ref(false); // 远程bin是否已同步
  const remoteBinSyncing = ref(false); // 远程bin是否正在同步中
  ...
  const updateToken = (tokenId: string, updates: Partial<TokenData>) => {
    ...
    const index = gameTokens.value.indexOf(token);
    gameTokens.value[index] = {
      ...
      server: updates.server ?? token.server,
      wsUrl: updates.wsUrl !== undefined ? updates.wsUrl : token.wsUrl,
    ...
    try {
      ...
      if (message.error) {
        ...
        if (errText.includes("token") && errText.includes("expired")) {
          ...
          const gameToken = gameTokens.value.find((t) => t.id === tokenId);
          const refreshSuccess = gameToken
            ? await attemptTokenRefresh(tokenId)
            : false;
          if (!refreshSuccess) {
            wsLogger.error(
  ...
  // Token刷新（从本地IndexedDB bin重新生成）
  const refreshTokenFromBin = async (token: TokenData): Promise<TokenRefreshResult> => {
    try {
      let buffer: ArrayBuffer | null = await getArrayBuffer(token.id);
      if (!buffer) {
      ...
      updateToken(token.id, { token: refreshedToken, updatedAt: new Date().toISOString() });
  ...
  const createWebSocketConnection = async (
    ...
    if (
      targetToken &&
      (targetToken.importMethod === "bin" || targetToken.importMethod === "wxQrcode") &&
      Date.now() - getTokenTimestamp(targetToken) >= TOKEN_REFRESH_THRESHOLD
    ) {
      const refreshed = await refreshTokenFromBin(targetToken);
      if (refreshed.success) {
        const latest = gameTokens.value.find((t) => t.id === tokenId);
        if (latest) base64Token = latest.token;
      }
    }
  ...
  const initTokenStore = () => {
    ...
    refreshTokensIfNeeded().catch((error) =>
      wsLogger.error("Token自动刷新失败", error),
    );
    // 启动连接监控
   ...
   * 从bin数据派生角色名称
   */
  const parseBinMetaFromFilename = (
    fileName: string,
  ): { roleName: string; server: string } => {
    if (!fileName || typeof fileName !== "string") {
      return { roleName: "", server: "" };
    }

    // Remove path and optional timestamp prefix like 1741440000000_
    const baseName = fileName
      .trim()
      .replace(/^.*[\\/]/, "")
      .replace(/^\d{10,}_/, "");

    // bin-{server}服-{index}-{roleId}-{roleName}.bin
    const matched = baseName.match(
      /^bin-(.*?)\u670d-([0-2])-([0-9]{6,12})-(.+)\.(bin|dmp)$/i,
    );
    if (matched) {
      return {
        server: matched?.[1]?.trim() || "",
        roleName: matched?.[4]?.trim() || "",
      };
    }
  ...
  const parseRoleNameFromBinFilename = (fileName: string): string => {
    return parseBinMetaFromFilename(fileName).roleName;
  };

  const normalizeServerLabel = (value: string): string => {
    const text = (value || "").trim();
    if (!text) return "";
    return text.endsWith("\u670d") ? text : `${text}\u670d`;
  };

  const deriveServerFromBin = (bin: any): string => {
    const fromOriginal = parseBinMetaFromFilename(bin?.originalName).server;
    if (fromOriginal) {
      return normalizeServerLabel(fromOriginal);
    }

    const fromFilename = parseBinMetaFromFilename(
      bin?.filename || bin?.storedName,
    ).server;
    if (fromFilename) {
      return normalizeServerLabel(fromFilename);
    }

    const directServer =
      typeof bin?.server === "string" ? bin.server.trim() : "";
    if (directServer) {
      return normalizeServerLabel(directServer);
    }

    return "\u672a\u77e5\u670d\u52a1\u5668";
  };

  const deriveRoleNameFromBin = (bin: any): string => {
    // Prefer parsing original uploaded filename first.
    const byOriginalName = parseRoleNameFromBinFilename(bin?.originalName);
    if (byOriginalName) {
      return byOriginalName;
    }

    // Fallback to other filename fields.
    const byFilename = parseRoleNameFromBinFilename(
      bin?.filename || bin?.storedName,
    );
    if (byFilename) {
      return byFilename;
    ...
    const directName = typeof bin?.name === "string" ? bin.name.trim() : "";
    if (directName) {
      return directName;
    }
   ...
   * 从bin数据插入或更新token
   */
  const upsertTokenFromBin = (
    roleName: string,
    tokenValue: string,
    bin: any,
  ) => {
    const serverName = deriveServerFromBin(bin);
    ...
    if (existingToken) {
      ...
      if (
        ...
        existingToken.server = serverName;
      }
      existingToken.updatedAt = new Date().toISOString();
  ...
  const getTokenTimestamp = (token: TokenData) => {
    return new Date(token.updatedAt || new Date().toISOString()).getTime();
  };

  /**
   * 如果没有选中token，自动选择最新的token
   */
  const setDefaultTokenIfNeeded = () => {
    ...
    const sortedTokens = [...gameTokens.value].sort(
      (a, b) => getTokenTimestamp(b) - getTokenTimestamp(a),
    );
    ...
    try {
      ...
      for (const bin of filteredBins) {
        try {
          ...
          const upsertedToken = upsertTokenFromBin(roleName, tokenValue, bin);
          if (storeArrayBuffer && upsertedToken?.id) {
            // New cache key: token.id. Keep a legacy name key for compatibility fallback.
            await storeArrayBuffer(upsertedToken.id, buffer).catch(() => null);
            if (upsertedToken.name && upsertedToken.name !== upsertedToken.id) {
              await storeArrayBuffer(upsertedToken.name, buffer).catch(() => null);
            }
          }
          restoredCount++;
        } catch (error) {
