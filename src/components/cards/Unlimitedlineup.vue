...
import {
  buildLineupStorageKeySets,
  loadLineupSettings,
  saveLineupSettings,
} from "@/utils/lineupStorage.js";
...
const currentRoleIdentity = ref(null);

const draggedHeroId = ref(null);
const dragOverPosition = ref(null);

const buildLineupStorageContext = () => ({
  keySets: buildLineupStorageKeySets({
    authUser: authStore.user,
    token: tokenStore.selectedToken,
    role: currentRoleIdentity.value,
  }),
...
const updateCurrentRoleIdentity = (role) => {
  if (!role) return;
  const roleId = role?.roleId || role?.id || null;
  const roleName = role?.roleName || role?.name || null;
  if (!roleId && !roleName) return;
  currentRoleIdentity.value = { roleId, roleName };
};
...
const loadSavedLineups = async () => {
  try {
    ...
    const result = await loadLineupSettings(buildLineupStorageContext());
    savedLineups.value = normalizeSavedLineups(result?.data);
  } catch (e) {
...
const saveLineupsToStorage = async () => {
  try {
    const token = tokenStore.selectedToken;
    if (!token) return;
    if (!currentRoleIdentity.value?.roleId) {
      message.warning("未获取到稳定角色ID，暂不保存阵容");
      return;
    }

    const result = await saveLineupSettings({
      ...buildLineupStorageContext(),
      data: savedLineups.value,
    });

    if (result?.remoteError && result?.localSaved) {
      message.warning("阵容已保存到本地，后端同步失败");
    } else if (result?.remoteError && !result?.localSaved) {
      throw result.remoteError;
    }
  } catch (e) {
...
const refreshTeamInfo = async () => {
  ...
  try {
    ...
    await delay(COMMAND_DELAY);
    const role = roleInfo?.role || roleInfo;
    updateCurrentRoleIdentity(role);
...
const applyLineup = async (lineup) => {
  ...
  const fetchLatestData = async (teamId = null) => {
    ...
    await delay(COMMAND_DELAY);
    updateCurrentRoleIdentity(roleInfo?.role || roleInfo);
  ...
  try {
    ...
    reader.onload = (e) => {
      try {
        ...
        const processImport = async (lineupsToImport) => {
        ...
        if (importData.roleId !== currentRoleId) {
          dialog.warning({
            ...
            negativeText: "取消",
            onPositiveClick: async () => {
              await processImport(importData.lineups);
            },
          });
        } else {
          void processImport(importData.lineups);
        }
      } catch (parseError) {
...
watch(
  ...
  (newToken, oldToken) => {
    if (newToken && newToken.id !== oldToken?.id) {
      currentRoleIdentity.value = null;
...
watch(
  () => currentRoleIdentity.value?.roleId || currentRoleIdentity.value?.roleName,
