...
const normalizeServerValue = (value) => {
  ...
  return String(value).replace(/服/g, "").trim();
};

const toServerLabel = (value) => {
  const normalized = normalizeServerValue(value);
  return normalized ? `${normalized}服` : "";
};

const parseBinMetaFromName = (fileName = "") => {
  const baseName = String(fileName || "")
    .trim()
    .replace(/^.*[\\/]/, "")
    .replace(/^\d{10,}_/, "");
  const matched = baseName.match(
    /^bin-(.*?)(?:服|\\u670d)-([0-2])-([0-9]{6,12})-(.+)\.(bin|dmp)$/i,
  );
  if (!matched) {
...
const pickBestRemoteBin = (bins, token) => {
  if (!Array.isArray(bins) || !bins.length) return null;

  const tokenName = String(token?.name || "").trim();
  const tokenServer = normalizeServerValue(token?.server);
  ...
  const scored = bins.map((bin) => {
    ...
    const joinedName = nameCandidates.join(" | ");
    const meta = parseBinMetaFromName(nameCandidates[0] || "");
    const metaRoleName = String(meta.roleName || "").trim();
    const metaServer = normalizeServerValue(meta.server);
    const binName = String(bin?.name || "").trim();

    let score = 0;
    if (tokenName) {
      if (metaRoleName && metaRoleName === tokenName) score += 100;
      if (binName && binName === tokenName) score += 60;
      if (joinedName.includes(tokenName)) score += 40;
    }
    if (tokenServer && metaServer && tokenServer === metaServer) score += 30;

    const ts = new Date(bin?.updatedAt || bin?.createdAt || 0).getTime();
    return { bin, score, ts };
  });
  ...
  return scored[0]?.score > 0 ? scored[0].bin : null;
};

const refreshBinTokenFromRemote = async (token) => {
  if (!authStore.isAuthenticated || !authStore.token) {
    return { refreshed: false, reason: "no-auth" };
  }
  ...
  if (!binsResponse.ok) {
    throw new Error(`加载后端bin失败: ${binsResponse.status}`);
  }

  const binsPayload = await binsResponse.json();
  const bins = Array.isArray(binsPayload?.bins)
    ? binsPayload.bins
    : Array.isArray(binsPayload?.data?.bins)
      ? binsPayload.data.bins
      : Array.isArray(binsPayload?.data)
        ? binsPayload.data
        : [];

  const matchedBin = pickBestRemoteBin(bins, token);
  if (!matchedBin?.id) {
    return { refreshed: false, reason: "not-found" };
  }

  const downloadResponse = await fetch(`/api/bins/${matchedBin.id}/download`, {
    headers: {
      Authorization: `Bearer ${authStore.token}`,
    },
  });
  if (!downloadResponse.ok) {
    throw new Error(`下载后端bin失败: ${downloadResponse.status}`);
  }

  const buffer = await downloadResponse.arrayBuffer();
  const newToken = await transformToken(buffer);
  const parsedMeta = parseBinMetaFromName(
  ...
  await deleteArrayBuffer(token.name).catch(() => {});
...
const refreshToken = async (token) => {
  ...
  try {
    if (token.importMethod === "url") {
      ...
      if (token.importMethod === "bin" && authStore.isAuthenticated) {
        try {
          const remoteResult = await refreshBinTokenFromRemote(token);
          refreshedFromRemote = !!remoteResult?.refreshed;
        } catch (error) {
          remoteError = error;
          console.warn("从后端恢复bin失败，将尝试本地bin:", error);
        }
      }

      if (!refreshedFromRemote) {
        ...
        if (userToken) {
          ...
          if (remoteError) {
            message.warning("后端恢复失败，已回退本地bin刷新Token");
          } else {
            message.success("Token刷新成功");
          }
        } else if (remoteError) {
          ...
          throw new Error("未找到可用bin数据，无法重新获取Token");
        }
      } else {
        message.success("已从后端bin恢复并刷新Token");
      }
    } else {
      dialog.info({
        title: "重新获取Token",
